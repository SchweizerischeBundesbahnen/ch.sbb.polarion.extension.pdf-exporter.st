...
- `==` pins in `pyproject.toml` are for Renovate — `ruff`, `mypy`, `types-requests`, `PyPDF2` use exact pins so Renovate creates PRs that go through CI. Do not relax these to ranges. Range-pinned deps are upgraded by `uv lock --upgrade`.
